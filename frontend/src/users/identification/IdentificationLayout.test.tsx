...
when clicking on forgot password link in login modal`, async () => {
    await runWithAndWithoutOrganizationPrefix(async (generatedPaths, paths) => {
      ...
      expect(history.location.pathname).toEqual(generatedPaths.RESET_PASSWORD);
...
function renderIdentificationLayout(history, generatedPaths, paths) {
  if (!paths) {
