...
Ensuite vous pouvez installer le paquet Python `pre-commit` et installer le hook Git pré-commit, pour vous assurer que le code que vous allez commiter a passé les vérifications de lint :
...
Par défaut, le backend sera disponible sur [localhost:8000](http://localhost:8000) mais sans le frontend qui tourne il n'y aura rien d'affiché là-bas, étant donné que le chemin racine affiche ce qui est disponible sur le port 3000.

Vous pouvez cependant accéder à d'autres chemins servis par le backend, notamment l'API naviguable fournie par Django REST Framework sur [localhost:8000/api/](http://localhost:8000/api/), et le panel admin de Django pour gérer les objets de la base de données sur [localhost:8000/api/](http://localhost:8000/admin/).
...
Par défaut, le frontend sera disponible sur [localhost:3000](http://localhost:3000) mais vous devez accéder à l'application depuis [localhost:8000](http://localhost:8000) quand à la fois le backend et le frontend sont en fonctionnement, car le frontend est en fait servi à travers une vue particulière de l'app Django.
...
Vous devez construire les images Docker d'abord, et ensuite démarrer les containers :
...
Vous pourriez aussi les lancer en mode démon, auquel cas vous n'aurez pas acces au logs depuis le terminal d'où vous avez lancé la commande, et vous devrez arrêter les containers avec une commande supplémentaire :
...
Comme avec le mode non-Docker, le serveur du frontend est disponible sur [localhost:8000](http://localhost:3000), et celui du backend est disponible sur [localhost:8000](http://localhost:8000).
