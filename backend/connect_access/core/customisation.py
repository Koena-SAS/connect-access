...
def fork_app(label, local_folder_path, local_app_subpackage=None, logger=None):
    ...
    try:
        ...
        raise ValueError(f"There is no app with the label '{label}'")
    ...
    logger.info(f"Creating package {local_app_folder_path}")
    ...
    msg = "Replace the entry '{0}' with '{1}' in INSTALLED_APPS".format(
