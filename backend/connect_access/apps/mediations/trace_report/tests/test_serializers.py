...
(ContactEntityType, TraceType) = get_classes(
    "mediations.trace_report.choices", ["ContactEntityType", "TraceType"]
)
...
class TestSerializers:
    def test_trace_report_serializes_correctly_all_fields(self, authenticate):
    ...
    def test_trace_report_deserializes_correctly_all_fields(
        ...
        assert (
            ContactEntityType(trace_report.recipient_type).name in request_data.values()
        )
    ...
    def test_trace_report_by_mediation_request_serializes_correctly_all_fields(
        ...
        trace_report1, trace_report2, mediation_request_uuid, response = itemgetter(
            ...
            _execute_trace_report_by_mediation_request(
                permission="admin", auth=authenticate
            )
        )
        data = response.data
        ...
        parse(
            data[0]["contact_date"]
        )  # we test that parsing it to datime does not raise
